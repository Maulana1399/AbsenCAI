...
final class ParticipationImportValidator implements ImportValidator
{
    /**
    ...
    public function validate(array $rows, ImportContext $context): ImportSummary
    {
        ...
        foreach ($rows as $row) {
            ...
            if (trim($row->data['desa'] ?? '') === '') {
                $rowErrors[] = new ImportError($row->rowNumber, 'desa', 'Desa wajib diisi.');
            } elseif (($row->data['desa_id'] ?? null) === null) {
            ...
            if (trim($row->data['regu'] ?? '') !== '' && ($row->data['regu_id'] ?? null) === null) {
                $rowErrors[] = new ImportError($row->rowNumber, 'regu', "Regu '{$row->data['regu']}' tidak ditemukan.");
            }
