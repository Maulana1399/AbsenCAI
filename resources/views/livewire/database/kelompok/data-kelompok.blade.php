        ...
        <table class="w-full text-sm text-left text-zinc-700 dark:text-zinc-300">
            ...
            @foreach($daftarkelompok as $kelompok)
                <tr class="bg-white dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900">
                    <td class="px-6 py-2 font-medium text-zinc-900 dark:text-white">{{ $loop->iteration }}</td>
                    <td class="px-6 py-2">{{ $kelompok->kelompok_asal }}</td>
                    <td class="px-6 py-2">{{ $kelompok->desa->desa_asal ?? '-' }}</td>
                    <td class="px-6 py-2 space-x-2">
                        <flux:button wire:click="edit({{ $kelompok->id }})">Edit</flux:button>
                        <flux:button variant="danger" wire:click="delete({{ $kelompok->id }})">Delete</flux:button>
                    </td>
                </tr>
            @endforeach
            </tbody>
