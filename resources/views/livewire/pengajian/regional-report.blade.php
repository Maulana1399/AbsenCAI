<div class="space-y-6">
    ...
    @elseif (!empty($summary))
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-3 sm:p-4 text-center shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                <p class="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-white">{{ $summary['total_desa'] }}</p>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-3 text-center shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                <p class="text-base sm:text-lg font-bold text-zinc-700 dark:text-zinc-400">{{ $summary['total_desa'] - $summary['desa_hadir'] }}</p>
    ...
    <div class="flex gap-1 rounded-xl bg-zinc-100 p-1 dark:bg-zinc-800 overflow-x-auto">
        <button
            ...
            class="whitespace-nowrap flex-1 rounded-lg px-3 py-2 text-sm font-medium transition {{ $activeTab === 'summary' ? 'bg-white text-zinc-900 shadow-sm dark:bg-zinc-700 dark:text-white' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white' }}"
        >
            Ringkasan per Desa
        </button>
        <button
    ...
    @if ($activeTab === 'list')
        <div class="rounded-xl border border-zinc-200 bg-white p-4 sm:p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                @if ($selectedDesaId)
                    <button
                        type="button"
                        wire:click="showAll"
                        class="self-start sm:self-auto text-xs text-emerald-600 hover:text-emerald-700 dark:text-emerald-400"
                    >
                        Tampilkan Semua
                    </button>
                @endif
